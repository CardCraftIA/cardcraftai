...
MESSAGES = {
    'purchase_history_error': ('Purchase history is temporarily unavailable. Please try again.', 'O histórico de compras está temporariamente indisponível. Tente novamente.', 'El historial de compras no está disponible temporalmente. Inténtalo de nuevo.', '購入履歴を一時的に取得できません。再度お試しください。'),
    'catalog_audit_failed': ('Your analysis was saved, but its catalog audit could not be completed. No new credit is needed.', 'Sua análise foi preservada, mas o registro complementar do catálogo não pôde ser concluído. Nenhum novo crédito é necessário.', 'Tu análisis se conservó, pero no se pudo completar el registro del catálogo. No necesitas otro crédito.', '分析結果は保持されていますが、カタログの記録を完了できませんでした。追加のクレジットは不要です。'),
